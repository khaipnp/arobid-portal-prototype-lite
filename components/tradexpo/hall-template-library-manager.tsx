...
export function HallTemplateLibraryManager() {
  ...
  function handleSave(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (formMode === "create") {
      const nextTemplate: HallTemplate = {
        id: createMockId("hall-template"),
      ...
      <Dialog open={formOpen} onOpenChange={handleFormOpenChange}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {formMode === "create"
                ? "Create Hall Template"
                : "Edit Hall Template"}
            </DialogTitle>
            <DialogDescription>
              Required fields: Name, GLB render file, and thumbnail image.
            </DialogDescription>
          </DialogHeader>
