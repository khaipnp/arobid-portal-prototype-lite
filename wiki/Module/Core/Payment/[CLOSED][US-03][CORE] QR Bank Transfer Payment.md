...
- **Upstream — [US-01][CORE] Configure Platform Default Payment Methods**: determines whether this flow or VNPay redirect is used
- **Upstream — [US-02][CORE] Manage Bank Accounts**: primary account is used for QR generation
- **Downstream — Orders & Transactions Admin reconciliation flow**: not currently active because the Orders & Transactions module has moved to VNPay-only
- **Downstream — [US-02][CORE Orders] Customer Order History and Detail**: order created here is visible to customer
...
6. On success: Exhibitor is redirected to **Order Detail page** ([US-02][CORE Orders] Customer Order History and Detail) with a confirmation banner: *"Your transfer has been submitted. We'll verify your payment and notify you by email."*
...
**Post-rejection retry (order returns to `Pending Payment` from Orders & Transactions epic):**

- If Admin rejects and order returns to `Pending Payment`, the same QR Payment screen is accessible from [US-02][CORE Orders] Customer Order History and Detail via a **"Retry Payment"** button
...
3. **Exhibitor returns to the platform and clicks "I've Transferred":**
   ...
   - Exhibitor is redirected to **Order Detail** with an info banner: *"Your transfer has been submitted. We are verifying your payment and will notify you by email once confirmed."*
...
5. **Retry after Admin rejection (accessed from Order Detail):**
...
| AC-07 | Admin rejects payment and Exhibitor clicks "Retry" on Order Detail | Retry initiated | QR Payment screen loads with a fresh VietQR; same Order ID and amount; 72h expiry resets from rejection timestamp; booth is `Available` |
