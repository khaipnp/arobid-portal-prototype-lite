...
interface Props {
  status: ExpoStatus | TemplateDerivedStatus
  type?: "expo" | "template"
}

let { status, type = "expo" }: Props = $props()

const activeStyles = $derived(
  type === "expo"
    ? expoStatusStyles[status as ExpoStatus]
    : templateStatusStyles[status as TemplateDerivedStatus]
)
</script>

<Badge variant="outline" class={cn(activeStyles)}>
  {status}
</Badge>
